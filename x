...
-- Function to tween the camera to the target position with smoothing and easing
local function TweenToPosition(targetPosition)
    local cameraCFrame = Camera.CFrame
    local targetCFrame = CFrame.new(cameraCFrame.Position, targetPosition)

    -- TweenInfo setup based on Smoothness and EasingStyle
    local tweenInfo = TweenInfo.new(
        getgenv().CamLock.Normal.Smoothness, -- Duration of tween (smoothness)
        Enum.EasingStyle[getgenv().CamLock.EasingStyle], -- Easing style from configuration
        Enum.EasingDirection.Out -- Out easing direction for smoother transitions
    )
    
    -- Create and play the tween for smooth camera movement
    local tween = TweenService:Create(Camera, tweenInfo, {CFrame = targetCFrame})
    tween:Play()
end

-- Main RenderStepped loop for targeting and camlock
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        -- Check if target's health is low to stop targeting
        ...
        -- Identify the body part to target
        local BodyPart
        ...
        if BodyPart then
            ...
            if predictedPosition then
                -- Use TweenToPosition if smoothness is enabled, else set position directly
